...
const STRINGS = {
  en: {
    ...
    CLEAR_BUTTON: "Clear",
    SEARCH_RESULTS_FOUND: (count) => `${count} found`,
    ...
    LIKE_CARD_PROFILES_STAT: (p, s) => `Profiles: ${p} & Snippets: ${s}`,
    ...
    NOTICE_ACTIVE_PROFILE_SWITCHED: (name) => `Active profile: ${name}`,
    NOTICE_CANNOT_FIND_ORIGINAL_PROFILE:
      "Could not find original data for this profile.",
    NOTICE_PROFILE_RESTORED: (name) =>
    ...
    NOTICE_PROFILE_CREATED_SUCCESS: (name) =>
      `Profile "${name}" was created successfully.`,
    NOTICE_PROFILE_IMPORTED_SUCCESS: (mode) => `Profile ${mode}d successfully.`,
    ...
    NOTICE_SNIPPET_NAME_EXISTS: (name) =>
      `Snippet name "${name}" already exists.`,
    NOTICE_PROFILE_NAME_EXISTS: (name) =>
      `Profile name "${name}" already exists.`,
    NOTICE_PROFILE_UPDATED: (name) => `Profile "${name}" updated.`,
    NOTICE_SNIPPET_UPDATED: (name) => `Snippet "${name}" updated.`,
    NOTICE_SNIPPET_CREATED: (name) =>
      `Snippet "${name}" has been created successfully!`,
    NOTICE_PROFILE_CREATED_FROM_CSS: (name) =>
      `Profile "${name}" has been created successfully!`,
    NOTICE_NO_COLOR_HISTORY: "No color history to restore.",
    NOTICE_COLOR_RESTORED: (color) => `Restored: ${color}`,
    ...
    TOOLTIP_PIN_SNAPSHOT_DATE: (date) =>
    ...
    MODAL_DELETE_SNIPPET_TITLE: (name) => `Delete Snippet: ${name}`,
    MODAL_DELETE_SNIPPET_DESC:
      "Are you sure you want to delete this snippet? This action cannot be undone.",
    NOTICE_FPS_UPDATED: (value) => `Live Update FPS set to: ${value}`,
    ...
    NEW_PROFILE: "New",
    RESTORE_PROFILE_MODAL_TITLE: (name) => `Restore Profile: ${name}`,
    RESTORE_PROFILE_MODAL_DESC: (name) =>
      `Are you sure you want to restore "${name}" to its original colors? All your customizations for this profile will be lost.`,
    NOTICE_PROFILE_CREATED: (name) => `Profile "${name}" created successfully!`,
    MARKDOWN: "Markdown",
    TEST_NOTICE_SUCCESS: "This is a test for a success notice!",
    TEST_NOTICE_ERROR: "This is a test for an error notice!",
    TEST_NOTICE_WARNING: "This is a test for a warning notice!",
    NOTICES: "Notices",
  ...
  ar: {
    ...
    EXPORT_BUTTON_TOOLTIP: "تصدير الملف الشخصي النشط",
    IMPORT_BUTTON_TOOLTIP: "استيراد ملف شخصي جديد",
    ...
    PROFILE_EXISTS_NOTICE: (name) => `الملف الشخصي "${name}" موجودة بالفعل.`,
    ...
    ISSUES_LINK_TEXT: "هذا الرابط.",
    BUILT_IN_PROFILES: "ملفّات شخصيّة جاهزة",
    ...
    CLEAR_BUTTON: "تنظيف",
    SEARCH_RESULTS_FOUND: (count) => `تم العثور على ${count}`,
    ...
    LIKE_CARD_PROFILES_STAT: (p, s) => `ملفّات شخصيّة: ${p} & قصاصات: ${s}`,
    ...
    NOTICE_ACTIVE_PROFILE_SWITCHED: (name) => `الملف الشخصي النشط: ${name}`,
    NOTICE_CANNOT_FIND_ORIGINAL_PROFILE:
    ...
    NOTICE_PROFILE_IMPORTED_SUCCESS: (mode) => `تم استيراد الملف الشخصي بنجاح.`,
    ...
    NOTICE_SNIPPET_NAME_EXISTS: (name) => `اسم القصاصة "${name}" موجود بالفعل.`,
    NOTICE_PROFILE_NAME_EXISTS: (name) =>
      `اسم الملف الشخصي "${name}" موجود بالفعل.`,
    NOTICE_PROFILE_UPDATED: (name) => `تم تحديث الملف الشخصي "${name}".`,
    NOTICE_SNIPPET_UPDATED: (name) => `تم تحديث القصاصة "${name}".`,
    NOTICE_SNIPPET_CREATED: (name) => `تم إنشاء القصاصة "${name}" بنجاح!`,
    NOTICE_PROFILE_CREATED_FROM_CSS: (name) =>
      `تم إنشاء الملف الشخصي "${name}" بنجاح!`,
    NOTICE_NO_COLOR_HISTORY: "لا يوجد سجل ألوان لاستعادته.",
    NOTICE_COLOR_RESTORED: (color) => `تمت الاستعادة: ${color}`,
    NOTICE_TEXTBOX_EMPTY:
    ...
    MODAL_DELETE_SNIPPET_TITLE: (name) => `حذف القصاصة: ${name}`,
    MODAL_DELETE_SNIPPET_DESC:
      "هل أنت متأكد من رغبتك في حذف هذه القصاصة؟ لا يمكن التراجع عن هذا الإجراء.",
    NOTICE_FPS_UPDATED: (value) =>
    ...
    NEW_PROFILE: "جديد",
    RESTORE_PROFILE_MODAL_TITLE: (name) => `استعادة الملف الشخصي: ${name}`,
    RESTORE_PROFILE_MODAL_DESC: (name) =>
      `هل أنت متأكد من رغبتك في استعادة "${name}" إلى ألوانه الأصلية؟ ستفقد جميع تخصيصاتك للملف الشخصي هذا.`,
    NOTICE_PROFILE_CREATED: (name) => `تم إنشاء الملف الشخصي "${name}" بنجاح!`,
    MARKDOWN: "عناصر الماركدوان",
    TEST_NOTICE_SUCCESS: "هذا إشعار تجريبي للنجاح!",
    TEST_NOTICE_ERROR: "هذا إشعار تجريبي للخطأ!",
    TEST_NOTICE_WARNING: "هذا إشعار تجريبي للتحذير!",
    NOTICES: "الإشعارات",
...
const processNotice = (el) => {
  let noticeText = (el.textContent || "").toLowerCase();
  const testKeywordEl = el.querySelector(".cm-test-keyword");
  ...
  try {
    ...
    const liveRules = T.liveNoticeRules;
    const liveRuleType = T.liveNoticeRuleType;
    ...
    let finalBgColor =
      settings.profiles[settings.activeProfile].vars["--cm-notice-bg-default"];
    let finalTextColor =
      settings.profiles[settings.activeProfile].vars[
...
class ColorMaster extends Plugin {
  ...
  async onload() {
    ...
    this.noticeObserver = new MutationObserver((mutations) => {
      ...
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          ...
          if (node.matches(".notice, .toast")) {
            processNotice(node);
          }
          node.querySelectorAll(".notice, .toast").forEach(processNotice);
        });
      });
    });
    ...
    document.querySelectorAll(".notice, .toast").forEach(processNotice);
  ...
  onunload() {
    ...
    this.removeInjectedCustomCss();
    this.stopColorUpdateLoop();
    console.log("Color Master v1.0.6 unloaded.");
  }
...
class ColorMasterSettingTab extends PluginSettingTab {
    ...
    rows.forEach((row) => {
      ...
      const displayName =
        this.plugin.settings.language === "ar"
          ? COLOR_NAMES_AR[varName] || snippetName
          : COLOR_NAMES[varName] || snippetName;
      const description =
        this.plugin.settings.language === "ar"
  ...
  _highlightRowMatches(row, state) {
    ...
    const descEl = row.querySelector(".setting-item-description");
...
<svg xmlns="http://www.w3.org/2000/svg" width="1280" height="240" viewBox="0 0 1280 240" role="img" aria-label="Color Master banner" class="cm-banner-svg">
  ...
  <text x="50%" y="84%" text-anchor="middle" font-family="Inter, Arial, sans-serif" font-size="18" fill="#FFFFFF" opacity="0.95">
    ...
    const snippetsCount = this._calcSnippetsCount();
    const totalItems = profilesCount + snippetsCount;
    ...
    this._createStatBar(
      ...
      this._calcPluginIntegrations(),
      5,
      "#8000ffff"
    );
    this._createStatBar(
      statsContainer,
      t("LIKE_CARD_DAYS_STAT"),
      days,
      365,
      "#ff0008ff"
    );
    ...
    for (const [category, vars] of Object.entries(DEFAULT_VARS)) {
      ...
      for (const [varName, defaultValue] of Object.entries(vars)) {
        ...
        const setting = new Setting(containerEl)
          .setName(
            this.plugin.settings.language === "ar"
              ? COLOR_NAMES_AR[varName] || varName
              : COLOR_NAMES[varName] || varName
          )
          .setDesc(description);
        ...
        if (category === "Notices") {
          setting.addExtraButton((button) => {
            button.setIcon("settings").onClick(() => {
              const ruleType = varName.includes("-text-")
                ? "text"
                : "background";
          ...
          button
            ...
            .onClick(async () => {
              ...
              if (history && history.length > 0) {
                ...
                new Notice(t("NOTICE_COLOR_RESTORED", restoredColor));
              } else {
...
class PasteCssModal extends Modal {
  ...
  onOpen() {
    ...
    if (this.isEditing && !this.existingSnippet.isProfile) {
...
class NoticeRulesModal extends Modal {
  constructor(app, plugin, settingTab, ruleType) {
    super(app);
    this.plugin = plugin;
    this.settingTab = settingTab;
  ...
  onOpen() {
    ...
    buttonContainer
      ...
      .addEventListener("click", async () => {
        const allTagInputs = this.rulesContainer.querySelectorAll(
          ".cm-tag-input-field"
        );
        ...
        const activeProfile =
          this.plugin.settings.profiles[this.plugin.settings.activeProfile];
        if (!activeProfile.noticeRules) activeProfile.noticeRules = {};
  ...
  displayRules() {
    ...
    this.localRules.forEach((rule, index) => {
      ...
      if (this.newlyAddedRuleId && rule.id === this.newlyAddedRuleId) {
        ...
        // Clear the flag so the animation doesn't repeat on the next re-draw
  ...
  // Fallback: basic HTML5 drag & drop (less smooth than SortableJS)
  _initDrag() {
    ...
    // Resolve Sortable constructor (imported or global)
    const S =
      typeof Sortable !== "undefined"
        ...
        ? require("sortablejs").default || require("sortablejs")
        : null;

    if (!S) {
      console.warn("Color Master: Sortable not found, drag & drop disabled.");
      return;
    }

    this.sortable = new S(this.rulesContainer, {
      ...
      onEnd: (evt) => {
        ...
        // Re-render to update numbers + rebind handlers
        this.displayRules();
      },
    });
  }
  ...
  _createTagInput(parentEl, rule) {
    ...
    const renderTags = () => {
      ...
      inputEl.placeholder = inputEl.placeholder = t("KEYWORD_PLACEHOLDER");
      ...
      const addKeywordFromInput = () => {
        ...
        if (newKeyword && !keywords.includes(newKeyword)) {
          ...
          const existingTagEl = container.querySelector(
            `.cm-tag-item[data-keyword="${newKeyword}"]`
          );
          if (existingTagEl) {
    ...
    container.addEventListener("click", () => {
      container.querySelector(".cm-tag-input-field")?.focus();
    });
...
class DuplicateProfileModal extends Modal {
  constructor(app, plugin, existingName, parsedProfile, settingTab, onSubmit) {
    super(app);
    this.plugin = plugin;
