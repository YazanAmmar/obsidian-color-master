export default {
  ...
  modals: {
    ...
    advancedReset: {
      ...
      settingsDesc:
        "بازنشانی زبان، FPS، چیدمان و سایر گزینه‌ها به حالت پیش‌فرض.",
    ...
    addLang: {
      ...
      rtlLabel: "زبان RTL",
      rtlDesc:
        "این گزینه را فعال کنید اگر زبان از راست به چپ نوشته می‌شود (مانند عبری، اردو).",
    },
    translator: {
  ...
  notices: {
    ...
    langCodeExists: (code: string) =>
      `یک زبان سفارشی با کد "${code}" از قبل وجود دارد.`,
    langNameExists: (name: string) =>
      `یک زبان سفارشی با نام "${name}" از قبل وجود دارد.`,
    ...
    deleteBackgroundsError: (message: string) =>
      `حذف پوشه پس‌زمینه‌ها ناموفق بود: ${message}`,
